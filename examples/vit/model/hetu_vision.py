...
class PatchMerger(ht.nn.Module):
    def __init__(self, config, ds_parallel_configs, name="patch_merger"):
        ...
        self.linear1 = ht.nn.HtMultiColumnParallelLinear(
            ...
            get_multi_ds_parallel_config(ds_parallel_configs, 'mlp_head'),
            bias=self.add_bias,
            gather_output=False,
            name=f'colp_{name}'
            # skip_bias_add=True
    ...
    def forward(self, x):
        # x = ht.reshape(x, [-1, self.config.num_patches, self.embed_dim])
        x = ht.mean(x, axes=[1])
        print("x", x.shape)
        x = self.linear1(x)
        return x
...
class VITModel(ht.nn.Module):

    def __init__(self, vision_config, ds_parallel_configs):
        super(VITModel, self).__init__()
        self.vision_config = vision_config
        self.ds_parallel_configs = ds_parallel_configs
        self.vision = VisionModel(vision_config,  get_multi_ds_parallel_config(ds_parallel_configs, 'vision'))
        self.patch_merger = PatchMerger(vision_config, get_multi_ds_parallel_config(ds_parallel_configs, 'patch_merger'))
    ...
    def forward(
        ...
        hidden_states = self.vision(image_inputs, position_ids, attention_mask, token_type_ids)


        print("hidden_states", hidden_states.shape)
        print("self.vision_config.micro_batch_size", self.vision_config.micro_batch_size)
        
        hidden_states = ht.reshape(hidden_states, [self.vision_config.micro_batch_size, -1, self.vision_config.embed_dim])
        lm_logits = self.patch_merger(hidden_states)
        
        # column parallel, [b * seq_len, n_embd] -> [b * seq_len, vocab_size], and splited in vocab dimension
        # lm_logits = self.lm_head(vision_hidden_states)
