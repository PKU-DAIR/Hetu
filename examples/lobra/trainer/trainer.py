...
class Trainer:
    ...
    def packed_run(self, args):
        ...
        if os.environ.get('EXPR_CUSTOM_DISTRIBUTION') == 'TRUE':
        ...
        if data_dispatch_pattern == 'GROUP':
            dynamic_planner = GroupDynamicDispatcher(self.cost_model, args.num_layers, train_task_num, global_batch_size_list,
                                                     self.strategy_config, self.max_tokens_list, local_device=local_device.index)
        elif data_dispatch_pattern == 'BALANCE':
            dynamic_planner = BalanceDynamicDispatcher(self.cost_model, args.num_layers, train_task_num, global_batch_size_list,
                                                     self.num_strategy, self.max_tokens_list, self.dps, self.tps, self.pps, local_device=local_device.index)
        else:
            raise ValueError(f"Unsupported data dispatch pattern: {data_dispatch_pattern}")
            ...
            for step in range(self.max_steps):
                ...
                for task_id in range(train_task_num):
                    if self.dataset_ctxs[task_id].step >= self.dataset_ctxs[task_id].steps or \
                        ...
                        continue
                    if train_iter_list[task_id] is None and os.environ.get('EXPR_CUSTOM_DISTRIBUTION') != 'TRUE':
                        continue
                    else:
                        if os.environ.get('EXPR_CUSTOM_DISTRIBUTION') == 'TRUE':
